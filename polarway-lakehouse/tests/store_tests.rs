...
async fn test_append_and_scan() {
    ...
    let batch = make_user_batch("u1", "albert", "albert@example.com");
...
async fn test_query_with_predicate() {
    ...
    // Insert two users
    let b1 = make_user_batch("u1", "albert", "albert@example.com");
    let b2 = make_user_batch("u2", "bernoulli", "bernoulli@example.com");
    store.append(schema::TABLE_USERS, b1).await.unwrap();
    store.append(schema::TABLE_USERS, b2).await.unwrap();

    // Query for albert
    let results = store
        .query(schema::TABLE_USERS, "username = 'albert'")
        .await
        .unwrap();
    ...
    // Query for bernoulli
    let results = store
        .query(schema::TABLE_USERS, "username = 'bernoulli'")
        .await
        .unwrap();
    let total: usize = results.iter().map(|b| b.num_rows()).sum();
    assert_eq!(total, 1);
...
async fn test_time_travel_by_version() {
    ...
    // Version 1: insert albert
    let b1 = make_user_batch("u1", "albert", "albert@example.com");
    store.append(schema::TABLE_USERS, b1).await.unwrap();

    // Version 2: insert bernoulli
    let b2 = make_user_batch("u2", "bernoulli", "bernoulli@example.com");
    store.append(schema::TABLE_USERS, b2).await.unwrap();
